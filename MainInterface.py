...
import sys, SearchModel1, GraphicalRun1, copy
...
class SearchModelInterface(object):
    ...
    def run_suite(self, target, distractors, search_type, num_distractors_list, num_runs):
        """
    ...
    def run_premade_simulation(self):
        ...
        if search_type == 'f':
            ...
            distractors = [['green', 'vertical', int(num_distractors/2)],['red', 'horizontal', int(num_distractors/2)]]
    ...
    def run_handmade_simulation(self):
        ...
        target_shape = input('Target shape >')
        ...
        while not done_with_lures:
            ...
            lure_number = int(input('How many '+lure_color+'-'+lure_shape+'s? >'))
    ...
    def run_handmade_suite(self):
        ...
        #num_distractor_list = []
        #while not done_with_num_distractors:
        #    num_distractors_input = input('Add a number of distractors to the simulation >')
        #    num_distractors_input = int(num_distractors_input)
        #    num_distractor_list.append(num_distractors_input)
        #    print('List of distractor numbers to run so far: ' + str(num_distractor_list))
        #    response = ''
        #    legal_responses = ('y','n')
        #    while not response in legal_responses:
        #        response = input('Add another number of distractors to the set of simulations to run? >')
        #        if response == 'n':
        #            done_with_num_distractors = True
        ...
        self.run_suite(target, distractors, search_text, num_distractor_list, num_runs)
        #self.run_suite(target, distractors, 'Conjunction', num_distractor_list, num_runs)

        #self.run_suite(['white', 'O'], [['white', 'Q']], 'Feature', [2, 4, 6, 8, 16], 200)
        #self.run_suite(['white', 'O'], [['black', 'Q']], 'Feature', [2, 4, 6, 8, 16], 200)
    ...
    def run_premade_suite(self):
        ...
        self.run_suite(['red', 'vertical'], [['green', 'horizontal']], 'Feature', [1, 5, 10, 20, 32], 1000)
        self.data_file_index += 1

        self.run_suite(['red', 'vertical'], [['green', 'vertical']], 'Feature', [2, 4, 10, 20, 32], 1000)
        self.data_file_index += 1

        self.run_suite(['red', 'vertical'], [['orange', 'vertical']], 'Feature', [1, 5, 10, 20, 32], 1000)
        self.data_file_index += 1

        self.run_suite(['red', 'vertical'], [['yellow', 'vertical']], 'Feature', [1, 5, 10, 20, 32], 1000)
        self.data_file_index += 1

        # self.run_suite(['red', 'vertical'], [['blue', 'vertical']], 'Feature', [1, 5, 10, 20, 32], 1000)
        # self.data_file_index += 1
        # # 3) Conjunction search: red vertical among green vertical & red horizontal
        #self.run_suite(['red', 'vertical'], [['green', 'vertical'], ['red', 'horizontal']], 'Conjunction', [2, 4, 10, 20, 32], 1000)
        #self.data_file_index += 1
        # # E) Search asymmetries
        self.run_suite(['white', 'Q'], [['white', 'O']], 'Feature', [1, 5, 10, 20, 32], 1000)
        self.data_file_index += 1
        self.run_suite(['white', 'O'], [['white', 'Q']], 'Feature', [1, 5, 10, 20, 32], 1000)
        self.data_file_index += 1
...
#     response = input('Use graphics? (y/n)')
# if response == 'y':
